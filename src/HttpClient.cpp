...
HttpClient::HttpClient() : m_ssl_ctx(bssl::context::tlsv12_client) {
...
std::string HttpClient::post(std::string_view host, std::string_view target,
  ...
  bhttp::request<bhttp::string_body> req{bhttp::verb::post, target,
                                         kHttpVersion};
...
std::string HttpClient::post_multipart(std::string_view host,
  ...
  req.set(bhttp::field::user_agent, kUserAgent);
  req.set(
      bhttp::field::content_type,
      std::string("multipart/form-data; boundary=") + std::string(boundary));
  req.body() = std::string(body);
  req.prepare_payload();
  return execute(req, host);
}
...
std::string HttpClient::get(std::string_view host, std::string_view target) {
  bhttp::request<bhttp::string_body> req{bhttp::verb::get, target,
                                         kHttpVersion};
  req.set(bhttp::field::host, host);
  req.set(bhttp::field::user_agent, kUserAgent);
  return execute(req, host);
...
std::string HttpClient::execute(bhttp::request<bhttp::string_body>& req,
                                std::string_view host) try {
  if (m_cancelled.load(std::memory_order_acquire)) {
    throw ex::NetworkException(boost::asio::error::make_error_code(

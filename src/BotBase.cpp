...
BotBase::EventData BotBase::wait_for_event() {
  ...
  if (ec == boost::asio::error::operation_aborted) {
    throw ex::InterruptedException("Прерывание при выполнении wait_for_event*()");
  }
  return result;
}
...
std::future<base::JsonType>
BotBase::send_request_async(const std::string &method,
                            const base::JsonType &params) {
  if (!m_authorized) {
    throw ex::NotConnectedException{};
  };
...
base::JsonType
BotBase::fill_required_params(const base::JsonType &params) const {
  base::JsonType out = params;
  if (!out.contains("access_token"))
    out["access_token"] = m_access_token;
  if (!out.contains("group_id"))
    out["group_id"] = m_group_id;
  if (!out.contains("v"))
    out["v"] = std::string(base::VKBOT_API_VERSION);
  return out;
}
